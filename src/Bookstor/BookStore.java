...
//-----------------------------------------------------------------------------------
    public double priceSummery() {
        ...
        for (int i = 0; i < this.stores.length; i++) {
         ...
         if(bookCount(stores[i].getType()) != 0) { totalSummary = totalSummary + priceSummery(stores[i].getType());}
        }
        return totalSummary;
    }
//-----------------------------------------------------------------------------------
boolean containsBookByNameAndType(String bookName,BookType bookType ) {
    ...
    boolean containsBookByName (String bookName) {
       boolean contains= false;
        for (int i = 0 ; i < this.stores.length ; i++){
            if (containsBookByNameAndType(bookName,stores[i].getType())){contains=true;}
        }
        return contains;
    }
//---------------------------------------------------------------------------------------------
   ...
   public Book []booksInOneType(BookType bookType){
       ...
       for (int i = 0; i <books.length ; i++) {
           ...
           bookInOneType[index]=books[i]; index++;

       }
         return bookInOneType;
   }
   public Book[]allBookInOneArray() {
       int index = 0;
...
//----------------------------------------------------------------------------------------
        ...
        for (int i = 0; i <books.length ; i++) {
         if (books[i].price <to && books[i].price>from){
             booksInPriceRange[index]=books[i];index++;
         }
        }
    ...
    public Book[]booksLessThen (double price){
        ...
        for (int i = 0; i <books.length ; i++) {
            if (books[i].price < price){
                booksLessThenPrice[index]=books[i];index++;}
        }
...
//-----------------------------------------------------------------------------------------
    public Book[]booksGreaterThan (double price){
        ...
        for (int i = 0; i <books.length ; i++) {
            if (books[i].price > price){booksGreaterThenPrice[index]=books[i];index++;}
        }
